...
OM_TEMPLATE_PATH = _resolve_path("OM_TEMPLATE_PATH", RESOURCES_DIR / "template_om.md")
OM_SECTIONS_PATH = _resolve_path("OM_SECTIONS_PATH", RESOURCES_DIR / "variables_om.json")
...
DEALS_DATA_DIR = DATA_ROOT / "deals"
...
# Database configuration (Postgres + pgvector for the OM generator)
_DEFAULT_LOCAL_DSN = "postgresql://postgres:PsQlP4sssh3rwood!@localhost:5432/vectordb"
_DATABASE_DSN = _normalise_database_dsn(
    os.getenv("DATABASE_DSN")
    or os.getenv("APP_DATABASE_DSN")
...
def _safe_folder_name(label: str) -> str:
    ...
    return cleaned or "deal"


def get_deal_storage_dirs(
    deal_id: str,
    *,
    company: Optional[str] = None,
    label: Optional[str] = None,
) -> Tuple[Path, Path, Path, Path, Path]:
    """Return base/input/output/assets/config directories for a deal."""

    ensure_storage_dirs()
    folder_label = label or company
    if folder_label:
        folder = f"{_safe_folder_name(folder_label)}-{deal_id}"
    else:
        folder = deal_id
    base = DEALS_DATA_DIR / folder
    inputs = base / "inputs"
    outputs = base / "outputs"
    assets = base / "assets"
    config_dir = base / "config"
    _ensure_dirs((base, inputs, outputs, assets, config_dir))
    return base, inputs, outputs, assets, config_dir
